...
class ConversationAnalysisForSimpleOutput(object):
    def __init__(
        self,
        summary: List[ConversationAnalysisSummaryItem],
        pii_analysis: List[List[ConversationAnalysisPiiItem]],
    ):
        self.summary = summary
...
def get_combined_redacted_content(channel: int) -> Dict:
    return {"channel": channel, "display": "", "itn": "", "lexical": ""}
...
def create_transcription(user_config: helper.Read_Only_Dict) -> str:
    ...
    response = rest_helper.send_post(
        uri=uri,
        content=content,
        key=user_config["speech_subscription_key"],
    ...
    try:
        ...
        raise Exception(
            f"Unable to parse response from Create Transcription API:{linesep}{response['text']}"
        )


def get_transcription_status(
    transcription_id: str, user_config: helper.Read_Only_Dict
) -> bool:
    uri = f"https://{user_config['speech_endpoint']}{SPEECH_TRANSCRIPTION_PATH}/{transcription_id}"
    response = rest_helper.send_get(
        uri=uri,
        key=user_config["speech_subscription_key"],
    ...
    if "failed" == response["json"]["status"].lower():
        raise Exception(
            f"Unable to transcribe audio input. Response:{linesep}{response['text']}"
        )
    else:
...
def get_transcription_uri(
    transcription_files: Dict, user_config: helper.Read_Only_Dict
) -> str:
    ...
    value = next(
        filter(
            lambda value: "transcription" == value["kind"].lower(),
            transcription_files["values"],
        ),
        None,
    )
    if value is None:
        raise Exception(
            f"Unable to parse response from Get Transcription Files API:{linesep}{transcription_files['text']}"
        )
...
def get_transcription(transcription_uri: str) -> Dict:
    response = rest_helper.send_get(
        uri=transcription_uri, key="", expected_status_codes=[HTTPStatus.OK]
    )
    return response["json"]


def get_transcription_phrases(
    transcription: Dict, user_config: helper.Read_Only_Dict
) -> List[TranscriptionPhrase]:
    def helper(id_and_phrase: Tuple[int, Dict]) -> TranscriptionPhrase:
        ...
        if "speaker" in phrase:
            ...
            raise Exception(
                f"nBest item contains neither channel nor speaker attribute.{linesep}{best}"
            )
...
def get_sentiment_analysis(
    phrases: List[TranscriptionPhrase], user_config: helper.Read_Only_Dict
    ...
    result_chunks = list(
        map(
            lambda xs: get_sentiments_helper(xs, user_config),
            helper.chunk(documents, 10),
        )
    )
...
def get_sentiments_for_simple_output(
    sentiment_analysis_results: List[SentimentAnalysisResult],
) -> List[str]:
    sorted_by_offset = sorted(
        sentiment_analysis_results, key=lambda x: x.offset_in_ticks
    )
    return list(map(lambda result: result.document["sentiment"], sorted_by_offset))


def get_sentiment_confidence_scores(
    sentiment_analysis_results: List[SentimentAnalysisResult],
) -> List[Dict]:
    sorted_by_offset = sorted(
        sentiment_analysis_results, key=lambda x: x.offset_in_ticks
    ...
    return list(
        map(lambda result: result.document["confidenceScores"], sorted_by_offset)
    )


def merge_sentiment_confidence_scores_into_transcription(
    transcription: Dict, sentiment_confidence_scores: List[Dict]
) -> Dict:
...
def transcription_phrases_to_conversation_items(
...
def request_conversation_analysis(
    conversation_items: List[Dict], user_config: helper.Read_Only_Dict
) -> str:
...
def get_conversation_analysis_status(
    conversation_analysis_url: str, user_config: helper.Read_Only_Dict
) -> bool:
    response = rest_helper.send_get(
        uri=conversation_analysis_url,
        key=user_config["language_subscription_key"],
        expected_status_codes=[HTTPStatus.OK],
    )
    if "failed" == response["json"]["status"].lower():
        raise Exception(
            f"Unable to analyze conversation. Response:{linesep}{response['text']}"
        )
    else:
        return "succeeded" == response["json"]["status"].lower()


def wait_for_conversation_analysis(
    conversation_analysis_url: str, user_config: helper.Read_Only_Dict
) -> None:
    done = False
    while not done:
        ...
        sleep(WAIT_SECONDS)
        done = get_conversation_analysis_status(
            conversation_analysis_url, user_config=user_config
        )
...
def get_conversation_analysis_for_simple_output(
    conversation_analysis: Dict, user_config: helper.Read_Only_Dict
) -> ConversationAnalysisForSimpleOutput:
    tasks = conversation_analysis["tasks"]["items"]

    summary_task = next(
        filter(lambda task: "summary_1" == task["taskName"], tasks), None
    )
    if summary_task is None:
        raise Exception(
            f"Unable to parse response from Get Conversation Analysis API. Summary task missing. Response:{linesep}{conversation_analysis}"
        )
    conversation = summary_task["results"]["conversations"][0]
    summary_items = list(
        map(
            lambda summary: ConversationAnalysisSummaryItem(
    ...
    if pii_task is None:
        raise Exception(
            f"Unable to parse response from Get Conversation Analysis API. PII task missing. Response:{linesep}{conversation_analysis}"
        )
    conversation = pii_task["results"]["conversations"][0]
    pii_items = [
        [
            ConversationAnalysisPiiItem(entity["category"], entity["text"])
...
def get_simple_output(
    phrases: List[TranscriptionPhrase],
    sentiments: List[str],
    conversation_analysis: ConversationAnalysisForSimpleOutput,
) -> str:
    result = ""
    for index, phrase in enumerate(phrases):
        ...
        result += reduce(
            ...
            conversation_analysis.summary,
            f"Conversation summary:{linesep}",
        )
    return result
...
def print_simple_output(
    ...
    conversation = get_conversation_analysis_for_simple_output(
        conversation_analysis, user_config
    )
    print(get_simple_output(phrases, sentiments, conversation))


def get_conversation_analysis_for_full_output(
    phrases: List[TranscriptionPhrase], conversation_analysis: Dict
) -> Dict:
    # Get the conversation summary and conversation PII analysis task results.
    tasks = conversation_analysis["tasks"]["items"]
    conversation_summary_results = next(
        filter(lambda task: "summary_1" == task["taskName"], tasks)
    )["results"]
    conversation_pii_results = next(
        filter(lambda task: "PII_1" == task["taskName"], tasks)
    )["results"]
    ...
    conversation["conversationItems"] = sorted(
        conversation["conversationItems"], key=lambda item: int(item["id"])
    )
    combined_redacted_content = [
        get_combined_redacted_content(0),
        get_combined_redacted_content(1),
    ]
    for index, conversation_item in enumerate(conversation["conversationItems"]):
        ...
        combined_redacted_content[channel][
            "lexical"
        ] += f"{redacted_content['lexical']} "
...
def print_full_output(
    ...
    transcription: Dict,
    sentiment_confidence_scores: List[Dict],
    phrases: List[TranscriptionPhrase],
    # conversation_analysis: Dict,
) -> None:
    results = {
        "transcription": merge_sentiment_confidence_scores_into_transcription(
            transcription, sentiment_confidence_scores
        ),
        # "conversationAnalyticsResults": get_conversation_analysis_for_full_output(
...
"""
    ...
    if False:  # user_config_helper.cmd_option_exists("--help"):
        print(usage)
    else:
        ...
        sentiment_confidence_scores = get_sentiment_confidence_scores(
        ...
        # NOTE: Conversation summary is currently commented out due to this reason
        # conversation_analysis_url = request_conversation_analysis(
        #     conversation_items, user_config
        # )
        # wait_for_conversation_analysis(conversation_analysis_url, user_config)
        # conversation_analysis = get_conversation_analysis(
        #     conversation_analysis_url, user_config
        # )
        # print_simple_output(
